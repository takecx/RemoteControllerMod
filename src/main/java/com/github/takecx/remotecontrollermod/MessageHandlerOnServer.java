...
public class MessageHandlerOnServer {
   ...
   */
  public static void onMessageReceived(final MoveCameraMessageToClient message, Supplier<NetworkEvent.Context> ctxSupplier) {
  ...
  static void processMessage(MoveCameraMessageToClient message, ServerPlayerEntity sendingPlayer)
  {
...
//
//      MoveCameraMessageToClient msg = new MoveCameraMessageToClient(message.getTargetCoordinates());
//    RegistryKey<World> playerDimension = sendingPlayer.func_241141_L_();  // func_241141_L_ is getPlayerDimension
//    Remotecontrollermod.simpleChannel.send(PacketDistributor.DIMENSION.with(() -> playerDimension), msg);
//
